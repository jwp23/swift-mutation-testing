...
extension OutputCaptureSerializedTests {
    ...
    struct MutantExecutorTests {
        ...
        @Test(
            "Given simulator pool setUp throws, when execute called, then pool tearDown runs and deletes tracked clones"
        )
        ...
        @Test(
            "Given main build fails and fallback build succeeds, when execute called, then mutant is not marked unviable"
        )
        ...
        @Test(
            "Given SPM build fails with canonical path in error, when retry succeeds, then only failing file is unviable"
        )
        ...
        @Test(
            "Given SPM project type with testTarget, when execute called, then testTarget is used in baseline validation"
        )
        ...
        @Test(
            "Given SPM build fails twice with error paths, when retry recurses, then second retry excludes more mutants"
        )
        ...
        func allCachedWithUnchangedTestFilesReturnsCachedResults() async throws {
            ...
            await cacheStore.store(
                status: .killed(by: "SomeTest"), for: cacheKey, killerTestFile: "Tests/SomeTest.swift")
            try await cacheStore.persist()
        ...
        @Test(
            "Given all mutants cached, when execute called with quiet false, then reporter shows loaded from cache count"
        )
