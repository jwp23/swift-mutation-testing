...
Resolution strategy: extracts the class or function name from the test name, then searches the candidate files for one whose final filename exactly matches `\(className).swift` (XCTest) or whose content contains `func \(functionName)` or both `@Test` and the function name (Swift Testing).
